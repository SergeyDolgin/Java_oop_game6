...
public class View {
    ...
    private static String getChar(int x, int y){
        String out = "| ";
        for (BaseHero baseHero: Main.listResult) {
            if (baseHero.getCoords()[0] == x && baseHero.getCoords()[1] == y){
                if (baseHero.getHP() == 0) {
                    out = "|" + (AnsiColors.ANSI_RED + baseHero.getInfo().charAt(0) + AnsiColors.ANSI_RESET);
                    break;
                }
                if (Main.team1.contains(baseHero)) out = "|" + (AnsiColors.ANSI_GREEN + baseHero.getInfo().charAt(0) + AnsiColors.ANSI_RESET);
                if (Main.team2.contains(baseHero)) out = "|" + (AnsiColors.ANSI_BLUE + baseHero.getInfo().charAt(0) + AnsiColors.ANSI_RESET);
                break;
            }
        }
        return out;
    ...
    public static void view() {
        ...
        step++;
        Main.listResult.forEach((v) -> l[0] = Math.max(l[0], v.toString().length()));
